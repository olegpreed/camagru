...
class User extends Model
{
	...
	/**
	 * Update user profile (username only, email requires verification)
	 * 
	 * @param int $userId
	 * @param string $username
	 * @param int $commentNotifications
	 * @return bool
	 */
	public function updateProfile(int $userId, string $username, int $commentNotifications): bool
	{
		$sql = "UPDATE {$this->table} 
                SET username = :username, comment_notifications = :comment_notifications, updated_at = NOW() 
                WHERE id = :id";
	...
	/**
	 * Set pending email change with verification token
	 * 
	 * @param int $userId
	 ...
	 * @return bool
	 */
	public function setPendingEmail(int $userId, string $newEmail, string $token, string $expiresAt): bool
	{
		$sql = "UPDATE {$this->table} 
                SET pending_email = :pending_email, 
                    pending_email_token = :token, 
                    pending_email_token_expires_at = :expires_at,
                    updated_at = NOW()
                WHERE id = :id";
		...
		try {
			...
			error_log("Failed to set pending email: " . $e->getMessage());
			return false;
		}
	}
	 ...
	 * @return array|null
	 */
	public function findByPendingEmailToken(string $token): ?array
	{
		$stmt = $this->db->prepare(
			"SELECT * FROM {$this->table} 
	...
	/**
	 * Confirm email change by applying pending email
	 * 
	 * @param int $userId
	 * @return bool
	 */
	public function confirmEmailChange(int $userId): bool
	{
		$sql = "UPDATE {$this->table} 
                SET email = pending_email,
                    ...
                    updated_at = NOW()
                WHERE id = :id AND pending_email IS NOT NULL";

		try {
			$stmt = $this->db->prepare($sql);
			return $stmt->execute(['id' => $userId]);
		} catch (PDOException $e) {
			error_log("Failed to confirm email change: " . $e->getMessage());
			return false;
		}
	}
	 ...
	 * @return bool
	 */
	public function clearPendingEmail(int $userId): bool
	{
		$sql = "UPDATE {$this->table} 
		...
		try {
			...
			error_log("Failed to clear pending email: " . $e->getMessage());
			return false;
		}
	}
}
